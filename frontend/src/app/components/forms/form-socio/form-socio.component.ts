...
export class FormSocioComponent {
  ...
  CargarFormulario(id: any) {
    this.servicioUsuario.getUsuarioPorId(id).subscribe(
      result => {
  ...
  cargarRol() {
    this.ArrayRoles = new Array<Rol>();
    this.servicioRol.getRoles().subscribe(
      result => {
  ...
  ActualizarUsuario() {
    this.servicioUsuario.updateUsuario(this.usuario).subscribe(
      result => {
        console.log(result);
        if (result.status == 1) {
  ...
  RegistrarUsuario(){
     this.servicioUsuario.addUsuario(this.usuario).subscribe(
      result => {
        console.log(result);
        if (result.status == 1) {
